        ...
        @elseif(!$isPending)
            ...
            <form class="form-grid" action="{{ $safeRoute('app.identity.store') }}" method="post" enctype="multipart/form-data" data-loading-form data-telegram-auth>
                ...
                <div class="field-row">
                    ...
                    <div class="field"><label class="field-label required" for="card-number">{{ $isFa ? 'شماره کارت بانکی' : 'Bank card number' }}</label><input class="input number ltr" id="card-number" name="card_number" inputmode="numeric" autocomplete="cc-number" pattern="[0-9۰-۹ ]{16,19}" maxlength="19" required value="{{ old('card_number') }}" placeholder="0000 0000 0000 0000"><p class="field-help">{{ $isFa ? 'کارت باید متعلق به همان کد ملی باشد.' : 'The card must belong to the same identity.' }}</p></div>
                </div>
                <div class="two-column">
                    <div class="field"><span class="field-label {{ $needsCorrection ? '' : 'required' }}">{{ $isFa ? 'تصویر کارت ملی' : 'National ID image' }}</span><label class="upload-box" for="national-card-image"><input id="national-card-image" name="national_id_image" type="file" accept="image/jpeg,image/png,image/webp" capture="environment" @required(!$needsCorrection) data-preview-input="#national-card-preview"><span class="upload-box-content"><span class="quick-icon"><x-icon name="upload" /></span><strong>{{ $needsCorrection ? ($isFa?'تعویض در صورت نیاز':'Replace only if needed') : ($isFa ? 'انتخاب یا گرفتن عکس' : 'Choose or take a photo') }}</strong><small class="muted">{{ $isFa ? 'چهار گوشه کارت و نوشته‌ها واضح باشد.' : 'Show all four corners with readable text.' }}</small></span><img class="upload-preview" id="national-card-preview" alt=""></label></div>
                ...
                <label class="checkbox"><input type="checkbox" name="consent" value="1" required @checked(old('consent'))><span>{{ $isFa ? 'صحت اطلاعات را تأیید می‌کنم و با بررسی مدارک برای فعال‌سازی پرداخت ریالی موافقم.' : 'I confirm the information is accurate and consent to document review for rial payments.' }}</span></label>
