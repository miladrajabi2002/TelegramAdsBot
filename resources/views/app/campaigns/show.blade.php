    ...
    <section class="section card" aria-labelledby="campaign-payment-title">
        ...
        <div class="two-column" style="align-items:start">
            ...
            <div class="stack-sm">
                ...
                @endif
                @if($nowPaymentsAvailable)<form class="option-card" action="{{ $safeRoute('app.campaigns.pay.nowpayments', ['campaign' => $id]) }}" method="post" data-loading-form data-telegram-auth>@csrf<span class="quick-icon"><x-icon name="globe" /></span><span class="option-card-copy"><strong>NOWPayments</strong><small>{{ $isFa?'ارز و شبکه را در فاکتور امن انتخاب می‌کنید.':'Choose currency and network inside the secure hosted invoice.' }}</small></span><button class="btn btn-sm btn-primary" type="submit">{{ $isFa ? 'فاکتور' : 'Invoice' }}</button></form>@endif
            </div>
        </div>
    </section>
@endif
