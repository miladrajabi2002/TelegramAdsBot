...
<section class="section" id="add-funds">
    ...
    <div class="two-column">
        <div class="card">
            <div class="card-head"><div><h3 class="card-title">ZarinPay</h3><p class="card-subtitle">{{ $isFa ? 'پرداخت ریالی با کارت بانکی تأییدشده' : 'Rial payment with a verified bank card' }}</p></div><x-icon name="card" class="text-primary" /></div>
            ...
            @elseif(!$canRial)
                ...
                <a class="btn btn-primary btn-block" style="margin-top:14px" href="{{ $safeRoute('app.identity.show') }}">{{ $isFa ? 'شروع احراز هویت' : 'Start verification' }}</a>
            @else
        ...
        <div class="card">
            <div class="card-head"><div><h3 class="card-title">NOWPayments</h3><p class="card-subtitle">{{ $isFa ? 'پرداخت رمزارزی با فاکتور زمان‌دار' : 'Crypto payment with an expiring invoice' }}</p></div><x-icon name="globe" class="text-primary" /></div>
