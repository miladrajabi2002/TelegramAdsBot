...
<div class="review-layout">
    ...
    <aside class="card">
        <div class="card-head"><div><h2 class="card-title">{{ $isFa?'چک‌لیست و تصمیم':'Checklist and decision' }}</h2><p class="card-subtitle">{{ $isFa?'برای تأیید همه موارد لازم است.':'Every item is required for approval.' }}</p></div></div>
        <form class="form-grid" action="{{ $safeRoute('admin.kyc.decision',['application'=>$id]) }}" method="post" data-loading-form>@csrf
            @if($cards->isNotEmpty())
                <div class="field"><label class="field-label required" for="review-card">{{ $isFa?'کارت مورد بررسی':'Card under review' }}</label><select class="select number ltr" id="review-card" name="card_id" required><option value="">{{ $isFa?'انتخاب کارت':'Choose a card' }}</option>@foreach($cards as $card)<option value="{{ data_get($card,'id') }}" @selected((string)old('card_id') === (string)data_get($card,'id'))>{{ data_get($card,'bin','••••••') }} •••••• {{ data_get($card,'last4','••••') }} — {{ data_get($card,'holder_name',data_get($card,'holder_name_search',$isFa?'نام صاحب کارت':'Card owner')) }}</option>@endforeach</select></div>
            @endif
            @foreach([
            ...
            <button class="btn btn-primary btn-block" name="decision" value="approved" type="submit" data-confirm="{{ $isFa ? 'هویت و کارت بانکی را با چک‌لیست بررسی کردید؟ پس از تأیید، پرداخت ریالی فعال می‌شود.' : 'Have you verified the identity and bank card against the checklist? Rial payments will be enabled.' }}"><x-icon name="check" />{{ $isFa?'تأیید احراز هویت':'Approve identity' }}</button>
            <button class="btn btn-danger btn-block" name="decision" value="changes_requested" type="submit" data-confirm="{{ $isFa ? 'این تصمیم به کاربر اطلاع داده می‌شود. ادامه؟' : 'The user will be notified. Continue?' }}"><x-icon name="edit" />{{ $isFa?'رد و درخواست اصلاح':'Reject and request changes' }}</button>
            <button class="btn btn-secondary btn-block" name="decision" value="manual_attention" type="submit" data-confirm="{{ $isFa ? 'این یادداشت ثبت شود و پرونده به اپراتور ارجاع داده شود؟' : 'Save this note and escalate for review?' }}"><x-icon name="warning" />{{ $isFa?'ارجاع برای بررسی بیشتر':'Escalate for review' }}</button>
            <button class="btn btn-danger btn-block" name="decision" value="rejected_permanent" type="submit" data-confirm="{{ $isFa?'این تصمیم حساب را برای پرداخت ریالی محدود می‌کند. مطمئن هستید؟':'This decision restricts rial payments. Are you sure?' }}"><x-icon name="warning" />{{ $isFa?'رد نهایی':'Reject permanently' }}</button>
        </form>
    </aside>
</div>
@endsection
