...
class PricingService
{
    ...
    {
        $rate = (float) $this->priceFeed->currentRates()['usd_irr'];
    ...
    public function irrToUsd(int $irr): float
    {
        ...
        return round(
            $irr / max(0.000001, $rate) * $this->conversionFactor(),
            2,
        );
    }
    ...
    public function quote(int $mediaBudgetIrr): array
    {
        ...
        // Exchange prices are always sourced from the live feed. There is no
        // manual exchange-rate switch in the admin panel anymore.
        $feed = $this->priceFeed->currentRates();
        $usdToIrr = (float) $feed['usd_irr'];
        $tonToUsd = (float) $feed['ton_usd'];
        ...
        return [
            ...
            'gram_amount' => round($usd / max(0.000001, $tonToUsd), 9),
            'usd_to_irr_rate' => $usdToIrr,
            'gram_to_usd_rate' => $tonToUsd,
            'conversion_margin_bps' => (int) round($conversionMarginPercent * 100),
            'rate_source' => (string) $feed['source'],
        ];
    }

    public function quoteTtlMinutes(): int
    {
        return min(1440, max(1, (int) $this->setting('quote_ttl_minutes', 15)));
    }

    public function minimumTargetMembers(): int
    {
        return max(1000, (int) $this->setting(
